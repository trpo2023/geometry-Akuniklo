...
int Check_Error(char *str)
{
    ...
    {
        if (!(isdigit(*(strchr(str, '(') + 1))) && !(*(strchr(str, '(') + 1) == '-') && !(*(strchr(str, '(') + 1) == '+'))
        {
            printf("%s", str);
        ...
        if (!(isdigit(*(strchr(str, ',') + 2))) && !(*(strchr(str, ',') + 1) == '+'))
        {
            printf("%s", str);
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < nCircle; i++)
    {
        ...
        for (int j = 0; j < nCircle; j++)
        {
            ...
            D = sqrt(pow(circle[i].point.x - circle[j].point.x, 2) + pow(circle[i].point.y - circle[j].point.y, 2));
